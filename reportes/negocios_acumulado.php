...
// Totales por año
try {
  $porAnio = $pdo->query("SELECT YEAR(created_at) AS anio, COUNT(*) AS total FROM negocios WHERE bestado = 1 AND created_at IS NOT NULL GROUP BY YEAR(created_at) ORDER BY anio DESC")->fetchAll();
} catch (Throwable $e) { $porAnio = []; }

// Totales por mes (año-mes)
try {
  $porMes = $pdo->query("SELECT YEAR(created_at) AS anio, MONTH(created_at) AS mes, COUNT(*) AS total FROM negocios WHERE bestado = 1 AND created_at IS NOT NULL GROUP BY YEAR(created_at), MONTH(created_at) ORDER BY anio DESC, mes DESC")->fetchAll();
} catch (Throwable $e) { $porMes = []; }

include __DIR__ . '/../partials/header.php';
?>
<div class="d-flex justify-content-between align-items-center mb-3">
  <h2>Reporte acumulado de negocios</h2>
  <a class="btn btn-outline-secondary" href="../index.php?a=negocios">Volver al listado</a>
</div>
